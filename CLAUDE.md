...
The migration is complete and the app is live and working at [https://bgmoffice.vercel.app](https://bgmoffice.vercel.app).
Login works against real data. A full review lives in `ROADMAP.md` — read it before making
architectural changes.

Security done:

- Row Level Security is ENABLED on all tables (the DB was previously reachable through
...
- **A table created at runtime by** `CREATE TABLE IF NOT EXISTS` **in application code does
not get RLS** — it skips the migration path where every other table got it. This was not
theoretical: `sms_messages` (created that way in `server/lib/smsStore.js`) had RLS off until
2026-09-15, which left every text the business had sent or received readable *and writable*
through Supabase's public REST API with the anon key — the key that ships in browsers and is
not a secret. Two Amber tables were the same. All three are fixed. If you add a table this
way, put `ALTER TABLE … ENABLE ROW LEVEL SECURITY` in the same statement that creates it.
- The security advisor should show zero `rls_disabled_in_public` findings. It will show many
`rls_enabled_no_policy` (INFO) — that is the intended state here (RLS on, no policy = owner
only), so do not "fix" those by writing policies. Check it with the Supabase MCP
`get_advisors`, and take Supabase's "Table publicly accessible" emails seriously; the one
Sarede forwarded on 2026-09-15 was real.
