...
// selected thread + a reply box. Polls every 12s so new inbound texts appear without a refresh.
...
export default function SmsPage() {
  ...
  return (
    <div className="mx-auto max-w-6xl px-3 py-4">
      <h1 className="mb-3 text-xl font-semibold text-gray-900">Texts</h1>
      <div className="flex h-[70vh] overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
        {/* Conversation list */}
        <aside className={`${active ? 'hidden md:block' : 'block'} w-full shrink-0 overflow-y-auto border-r border-gray-200 md:w-72`}>
          {loading ? (
            ...
            threads.map((t) => (
              <button
                ...
                className={`flex w-full flex-col gap-0.5 border-b border-gray-100 px-4 py-3 text-left hover:bg-gray-50 ${active === t.phone ? 'bg-blue-50' : ''}`}
              >
                <div className="flex items-center justify-between gap-2">
                  <span className="truncate font-medium text-gray-900">
                    {t.person_name || fmtPhone(t.phone)}
                  </span>
