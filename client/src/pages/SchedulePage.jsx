...
export default function SchedulePage() {
  ...
  // The class whose confirmation text is being previewed: who it's to, and which kind of
  // class row it is.
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-5">
                              ...
                              <div onClick={() => setSessionModal({ session: s })}
                                ...
                                {s.instructor_id && (
                                  <button onClick={e => { e.stopPropagation(); setTextClass({ who: 'instructor', row: s, kind: 'session' }) }}
                                    ...
                                    className={`w-full mt-1 text-[10px] rounded px-1 py-0.5 border transition-colors whitespace-nowrap ${
                                      s.instructor_text_sent_at
                                        ? 'border-emerald-300 bg-emerald-50 text-emerald-700 hover:bg-emerald-100'
                                        : 'border-gray-200 text-gray-500 hover:bg-gray-50'
                                    }`}>
                                    {s.instructor_text_sent_at ? '✓ Texted instructor' : 'Text Instructor'}
                                  </button>
                                )}
                                {s.instructor_id && (
                  ...
                  <div className={`flex items-center gap-3 px-4 py-3 ${i > 0 ? 'border-t border-gray-100' : ''} ${s.status === 'paused' ? 'opacity-50' : ''}`}>
                    ...
                    {s.instructor_id && (
                      <button
                        ...
                        title="Text the instructor a short class confirmation — they still get the full email"
                        className={`text-xs rounded-lg px-2 py-1 border transition-colors ${
                          s.instructor_text_sent_at
                            ? 'border-emerald-300 bg-emerald-50 text-emerald-700 hover:bg-emerald-100'
                            : 'border-gray-200 text-gray-500 hover:bg-gray-50'
      ...
      {textClass && (
        <TextConfirmModal
          who={textClass.who}
          classRow={textClass.row}
          kind={textClass.kind}
          onClose={() => setTextClass(null)}
          onSent={r => {
            const stamp = x => (textClass.who === 'client'
              ? { ...x, client_text_sent_at: r.sent_at, client_text_sent_to: r.sent_to }
              : { ...x, instructor_text_sent_at: r.sent_at, instructor_text_sent_to: r.sent_to })
            if (textClass.kind === 'session') {
              setSessions(prev => prev.map(x => x.id === textClass.row.id ? stamp(x) : x))
            } else {
              setSchedules(prev => prev.map(x => x.id === textClass.row.id ? stamp(x) : x))
            }
          }}
        />
      )}
