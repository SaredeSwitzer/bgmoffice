...
// Preview + send a "your class time/date changed" email for one dated session — separate
// from the initial confirmation email, for after a session gets edited or dragged to a
// different day/time and the instructor needs to actually find out about it.
export default function RescheduleAlertModal({ session, onClose, onSent }) {
  ...
  useEffect(() => {
    ...
    api.getRescheduleAlertPreview(session.id)
      .then(p => { if (!cancelled) { setPreview(p); setSubject(p.subject); setBody(p.body) } })
  ...
  async function send() {
    ...
    try {
      const r = await api.sendRescheduleAlert(session.id, { subject, body })
      onSent?.(r)
      onClose()
    } catch (e) {
